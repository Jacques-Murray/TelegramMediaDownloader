...
class TelegramMediaDownloader:
    ...
    async def download_all_unread_media(self, mark_as_read: bool = True) -> DownloadSession:
        """
    ...
    async def _process_channel(self, channel: Any, mark_as_read: bool) -> ChannelStats:
        ...
        try:
            ...
            media_messages = [
                ...
                if self.media_filter.should_download(msg)
            ]
            stats.media_count = len(media_messages)
            
            self.logger.info(f"Found {len(media_messages)} media messages in {channel_name}")
            
            # Download media from each message
            for message in media_messages:
                try:
                    media_info = await self.media_downloader.download_media_from_message(
            ...
            if mark_as_read and unread_messages:
                await self.channel_manager.mark_messages_as_read(channel, unread_messages)
    ...
    async def download_from_specific_channels(
        ...
        target_channels = [
            ...
            if getattr(ch, 'title', '') in channel_names
        ]
