...
class ChannelManager:
    ...
    async def get_unread_messages(self, channel: Any) -> List[Any]:
        ...
        try:
            ...
            async for message in client.iter_messages(channel.entity, limit=unread_count):
                messages.append(message)
            
            return messages
            
        except Exception as e:
            self.logger.error(f"Error getting unread messages from {channel.title}: {e}")
            return []
    ...
    async def mark_messages_as_read(self, channel: Any, messages: List[Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"Marked {len(messages)} messages as read in {channel.title}")
