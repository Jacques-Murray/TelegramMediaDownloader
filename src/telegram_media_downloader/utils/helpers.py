...
def print_session_summary(session: DownloadSession) -> None:
    ...
    if session.total_downloaded > 0:
        print(f"Average files per active channel: {session.average_files_per_channel:.1f}")
    ...
    if session.channel_stats:
        ...
        print(f"{'='*60}")
        print(f"{'Channel':<35} {'Unread':<8} {'Media':<8} {'Downloaded':<12} {'Errors':<8}")
        print(f"{'-'*75}")
        
        for stats in session.channel_stats:
            print(f"{stats.name[:34]:<35} {stats.unread_count:<8} "
...
def validate_channel_names(channel_names: List[str], available_channels: List[str]) -> List[str]:
    """
...
def print_file_statistics(download_path: Path) -> None:
    ...
    if stats['channels']:
        ...
        for channel, data in sorted(stats['channels'].items()):
            print(f"  {channel}: {data['files']} files ({format_file_size(data['size'])})")
...
def cleanup_empty_directories(download_path: Path) -> int:
    ...
    for dir_path in sorted(download_path.rglob('*'), key=lambda p: len(p.parts), reverse=True):
